...
var main_block_back_empties, annotation_name, block_name, inception_level, industry, sub_industries_annotation_ids, selected_industry, popup_text, object_name, sub_industries_objects, annotation_id, application_page, interior_scene, internal_text_content, header, home_button, back_button, help_button, percentage, main_blocks, selected_sub_industry, sub_industries, block, internal_text, ev_solution_annotations, internal_view, j, i, k, m;
...
function go_back(industry) {
  ...
  loadScene('boyd_smart_city.gltf', 'boyd_smart_city.gltf', function() {
    endPreloader();
    autoRotateCamera(true, 0.2);
    zoomCamera(getEntityByName(dictGet(block, industry), 'GROUP'), 2, function() {});
...
function hide_sub_industries_annotations() {
  setHTMLElemStyle('display', 'none', ['Avionics Thermal Chassis', 'Engine Heat Exchanger', 'In-Flight Entertainment HMI', 'Instrument Panel HMI', 'Flame Barrier Insulation', 'Industrial Solutions', 'Semiconductor Solutions', 'EV Charge Station Solutions', 'Vehicle Solutions', 'ADAS Compute Cooling', 'Medical Solutions', 'Aircraft Engineered Materials  ', 'Radar and Communications Thermals ', 'Satellite Thermal Control ', 'Electronics Thermal Chassis ', 'Land System Electronics Cooling ', 'Flame Barrier Insulation ', 'Data Center Solutions', 'Radio Base Station Rack Cooling', 'Renewable Cooling', 'Power Storage Cooling', 'Power Conversion Cooling'], false);
}
...
function set_css_rule_sub_annotation(annotation_id) {
  ...
  if (featureAvailable('MOBILE')) {
    setCSSRuleStyle('fontSize', '0.75rem', '.sub_annotation', false, '');
...
function startPreloader() {
  setHTMLElemStyle('filter', 'blur(24px)', ['QUERYSELECTOR', '#v3d-container > canvas'], false);
  registerSetTimeout(0.2, function() {
...
interior_scene = dictCreateWith(['Industrial Solutions', 'Medical Solutions', 'Vehicle Solutions', 'Data Center Solutions', 'Semiconductor Solutions'], ['industrial_internal.gltf', 'medical_internal.gltf', 'vehicle_solutions_internal.gltf', 'data_center_solutions_internal.gltf', 'semiconductor_internal.gltf']);
